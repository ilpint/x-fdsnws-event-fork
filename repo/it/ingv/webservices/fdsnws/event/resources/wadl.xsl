<!--
    ...
    Minor changes to support div and others text in doc.

    Stefano Pintore INGV
 -->
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:wadl="http://wadl.dev.java.net/2009/02" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="2.0">
...
<xsl:template match="wadl:strong">
<strong>
    <xsl:apply-templates/>
</strong>
</xsl:template>

<xsl:template match="wadl:ul">
<ul>
    <xsl:apply-templates/>
</ul>
</xsl:template>

<xsl:template match="wadl:li">
<li>
    <xsl:apply-templates/>
</li>
</xsl:template>

<xsl:template match="wadl:a">
<a>
                    ...
                    <xsl:for-each select="wadl:option">
                        <xsl:choose>
                            <xsl:when test="@mediaType">
                                <br/>
                                <tt>
                                    <xsl:value-of select="@value"/> (<xsl:value-of select="@mediaType"/>)</tt>
                            </xsl:when>
                            <xsl:otherwise>
                                <tt>
